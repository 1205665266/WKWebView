...
//    }];
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"ZZZ" style:(UIBarButtonItemStyleDone) target:self action:@selector(buttonAction)];
}

-(void)buttonAction
{
        [_bridge callHandler:@"textAction" data:@"oc调用js端方法" responseCallback:^(id responseData) {
            
            //
            NSLog(@"responseData===%@==",responseData);
        }];
//    _bridge callHandler:<#(NSString *)#> data:<#(id)#>
//    _bridge callHandler:<#(NSString *)#>
}
...
//        [XGTool callPhone:resourceSpecifier];
        //系统打电话
        NSString *num = [[NSString alloc]initWithFormat:@"tel:%ld",13012345678];
